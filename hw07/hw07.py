...
def deep_map_mut(fn, link):
    ...
    if not isinstance(link, Link):
        ...
        link.rest = deep_map_mut(link.rest)
...
def two_list(lst1, lst2):
    ...
    '''
    if len(lst1) < 2:
        return Link(lst1, empty)
    else:
        current = [lst1[0]] * lst2[0]
        lst1 = lst1[1:]
        lst1 = lst2[1:]
        return Link(current, two_list(lst1, lst2)) 
    '''
    def form_list(lit1, lst2):
        current = []
        return [[lst1[i]] * lst2[i] for i in range(len(lst1))]
    def form_linked_list():
        pass
...
def prune_min(t):
    ...
    """
    t.branches = [b.root for b in t.branches]
    for b in t.branches:
        prune_min(b)
