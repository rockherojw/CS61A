...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        exp = scheme_eval(first, env)
        check_procedure(exp)
        operators = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(exp, operators, env)
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        formal_symbols = Pair(binding.first, formal_symbols)
        vals = Pair(scheme_eval(binding.second.first, env), vals)
        bindings = bindings.second
        check_formals(formal_symbols)
    return env.make_child_frame(formal_symbols, vals)
    # END PROBLEM 15
   ## Alternative Solution ##
    ...
    bindings.map(lambda binding: child_env.define(binding.first, scheme_eval(binding.second.first, env)))
    return child_env'''
