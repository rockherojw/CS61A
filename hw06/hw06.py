...
def make_withdraw(balance, password):
    ...
    error_time = 0
    attempts = []
    def withdraw(money, input_pass):
        nonlocal balance, password, error_time

        while error_time < 3:
            if input_pass == password and money <= balance:
                balance -= money
                return balance
            elif input_pass == password and money > balance:
                return 'Insufficient funds'
            else:
                attempts.append(input_pass)
                error_time += 1
                return 'Incorrect password'
        return str('Your account is locked. Attempts:', attempts)
    return withdraw
...
class VendingMachine:
    ...
    def deposit(self, b):
        if self.stock == 0:
            return 'Machine is out of stock. Here is your $' + str(b) + '.'
        self.balance += b
    ...
    def vend(self):
        ...
        if self.balance - self.price != 0:    
            return 'Here is your ' + str(self.product) + ' and $' + str(self.price - self.balance) + ' change.'
        self.stock -= 1
        self.balance = 0
